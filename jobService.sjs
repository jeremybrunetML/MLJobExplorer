...
function get (context, params) {
    ...
    switch (params.mode) {
        case 'kpi':
            return getKpis();
        case 'collections':
            return cts.collections("*").toArray()
...
function post (context, params, input) {
    ...
    if (input.service == "jobs") {
        ...
        let total = fn.count(cts.search(cts.andQuery(andQuery)))
        let jobs = fn.subsequence(cts.search(cts.andQuery(andQuery), cts.indexOrder(cts.jsonPropertyReference(indexName), "descending")), ((input.page - 1) * input.page_size) + 1, input.page_size)
    ...
    if (input.service == "batchs") {
        ...
        if (input.type == 1) {
            ...
            let total = fn.count(cts.search(cts.andQuery([
                cts.collectionQuery("Batch")
                , cts.jsonPropertyValueQuery("jobId", jobId)])
            ))

            let batchs = fn.subsequence(cts.search(
                ...
                , [cts.indexOrder(cts.jsonPropertyReference("timeStarted"), "descending"), cts.scoreOrder("descending")]), ((input.page - 1) * input.page_size) + 1, input.page_size)
            ...
            let batchs = fn.subsequence(cts.search(cts.andQuery([
                cts.collectionQuery(batchCollection)
                , cts.jsonPropertyValueQuery("jobId", jobId)
                , cts.orQuery([
                    cts.trueQuery()
                ...
                , [cts.scoreOrder("descending")]), ((input.page - 1) * input.page_size) + 1, input.page_size)
